...
BAK_LOGS_PATH="/etc/codec/logs/mods.bak"

rm -rf $BAK_LOGS_PATH
mv $LOGS_PATH $BAK_LOGS_PATH
...
if [[ "$1" == "-q" || "$1" == "--quit" || "$1" == "quit" ]]; then
    for QUIT_MOD_PATH in $QUIT_MOD_PATHS; do
        ...
        QUIT_MOD_LOGS_PATH=$LOGS_PATH/$QUIT_MOD_NAME.log
...
for ENV_MOD_PATH in $ENV_MOD_PATHS; do
    ...
    ENV_MOD_LOGS_PATH=$LOGS_PATH/$ENV_MOD_NAME.log
...
source /codec/.codec/env.sh &> $LOGS_PATH/env.log
...
for BOOT_MOD_PATH in $BOOT_MOD_PATHS; do
    ...
    BOOT_MOD_LOGS_PATH=$LOGS_PATH/$BOOT_MOD_NAME.log
